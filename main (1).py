...
def startGame():
  ...
  tabx = 1400
  taby = 800
  pause_button = py.draw.rect(tab,(30,30,30), (1380,20,20,20))
  ...
  while on:
    ...
    if k[py.K_a] and x > v:
        x = x - v
    if k[py.K_s] and y < taby - h - v:
        y = y + v
    if k[py.K_d] and x < tabx - w - v:
        x = x + v
    if k[py.K_w] and y > v:
        y = y - v
    ...
    tab.fill((255,255,255))
    player = py.draw.rect(tab,(0,0,0), (x,y,w,h))
    pause_button = py.draw.rect(tab,(30,30,30), (1380,0,20,20)) 
    py.display.update()
